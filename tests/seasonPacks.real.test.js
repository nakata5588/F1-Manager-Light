...
test("1980-1985 Season Packs use exact R2B historical rating snapshots",async()=>{
  for(const year of [1980,1981,1982,1983,1984,1985]){
    ...
    assert.ok(ratings.some((row)=>String(row.source||"")==="historical_rating_snapshot_r2b"),year+" must contain R2B snapshot ratings");
    assert.equal(ratings.some((row)=>Number(row.year)!==year),false,year+" rating rows must be materialized for the selected New Game year");
  }
...
test("1981 Giacomelli has exactly one race-team assignment",async()=>{
  ...
  const rows=(pack.state?.contracts||[]).filter((row)=>isRaceDriverContract(row)&&String(row.driver_id)==="d_0152");
...
test("drivers who die during the selected season are alive on New Game January 1",async()=>{
  ...
  const orphan=(pack.state?.contracts||[]).filter((row)=>!(pack.state?.drivers||[]).some((d)=>String(d.driver_id)===String(row.driver_id)));
