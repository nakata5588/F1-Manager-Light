...
test("tab session recovery overrides a stale qualifying rolling save during refresh", () => {
  ...
  assert.equal(recovered.raceWeekendState.active_session_id,"race");
...
test("tab session recovery never crosses into a different career", () => {
  ...
  const other={
    ...
    raceWeekendState:{phase:"race",active_session_id:"race",live_race:{status:"running",current_lap:12,current_sector:1}},
  };
