...
export default function Standings() {
  ...
  const driversById = useMemo(() => new Map(driversDb.map((d) => [str(d?.driver_id ?? d?.id), d])), [driversDb]);
  const teamsById = useMemo(() => new Map(teamsDb.map((t) => [str(t?.team_id ?? t?.id), t])), [teamsDb]);
  const stats = useMemo(() => buildResultStats(results), [results]);
  ...
  const driverRows = useMemo(() => {
    ...
    return source
      .map((row) => {
        ...
        const teamId = str(row?.team_id ?? row?.constructor_id ?? db?.team_id ?? db?.constructor_id ?? db?.team);
        return {
          id,
  ...
  }, [standings, driversById, teamsById, stats]);
    ...
    <div className="grid gap-4">
      <div className="bg-white rounded-xl shadow p-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">Season {gameState?.activeYear ?? "—"}</p>
        </div>
