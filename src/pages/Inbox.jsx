...
export default function Inbox() {
  ...
  function markRead(message) {
    if (!message) return;
    if (message.origin === "event") {
      markNewsRead?.(message.sourceId);
      return;
    }
    const base = Array.isArray(gameState?.inbox) ? gameState.inbox : [];
    if (!base.some((msg) => String(msg.id) === String(message.sourceId))) return;
    setGameState({
      inbox: base.map((msg) => String(msg.id) === String(message.sourceId)
  ...
  function openItem(message) {
    ...
    if (message.unread) markRead(message);
  }
  ...
  return (
    <div className="-mx-3 -my-4 md:-mx-5 md:-my-5 min-h-[calc(100vh-4rem)] bg-[#090b10] text-slate-100 p-4 md:p-6 space-y-4">
      ...
      <div className="rounded-xl border border-white/10 bg-[#12141c] shadow-lg p-3">
        <div className="flex flex-col lg:flex-row lg:items-center gap-2">
          <select value={cat} onChange={(e) => setCat(e.target.value)} className="border border-white/10 bg-[#191c26] text-slate-100 rounded-md px-3 py-2 text-sm">
          ...
          <input
            ...
            className="border border-white/10 bg-[#191c26] text-slate-100 placeholder:text-slate-500 rounded-md px-3 py-2 text-sm flex-1"
          />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 min-h-[calc(100vh-12rem)]">
        <section className="xl:col-span-4 rounded-xl border border-white/10 bg-[#12141c] text-slate-100 shadow-lg overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between">
            <div className="text-sm font-semibold">Messages</div>
            <div className="text-xs text-slate-400">{filtered.length}</div>
          </div>
          <div className="flex-1 max-h-[calc(100vh-15rem)] overflow-y-auto divide-y divide-white/10">
        ...
        <section className="xl:col-span-8 rounded-xl border border-white/10 bg-[#12141c] text-slate-100 shadow-lg overflow-hidden">
          {active ? (
            <>
              ...
              <div className="p-5 md:p-7">
                ...
                <div className="mt-8 pt-5 border-t border-white/10 flex flex-wrap gap-2">
                  {(active.actions?.length ? active.actions : active.actionRoute ? [{ label: "Open related page", route: active.actionRoute }] : []).map((action, index) => (
                    <Link
                      key={action.route + "_" + index}
                      to={action.route}
                      className="inline-flex items-center rounded-md bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-white"
                    >
                      {action.label}
                    </Link>
                  ))}
                  {active.unread ? (
                    <Button variant="outline" onClick={() => markRead(active)}>Mark as read</Button>
                  ) : null}
                </div>
              </div>
            </>
